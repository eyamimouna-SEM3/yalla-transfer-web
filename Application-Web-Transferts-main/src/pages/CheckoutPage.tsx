...
const CheckoutPage = () => {
  ...
  const paymentMethods = useMemo(() => {
    const methods = [
      ...
      { id: "bank", label: "Virement bancaire", icon: Building2, desc: "Transfert bancaire" },
      { id: "post", label: "Poste (e-DINAR)", icon: Mail, desc: "La Poste Tunisienne" },
    ];
    if (hasBus) {
  ...
  const getBackendPaymentMethod = (
    method: string,
  ): 'cash' | 'card' | 'stripe' | 'paypal' | 'virement' => {
    switch (method) {
      case 'bank':
        return 'virement';
      case 'post':
        return 'card';
      default:
        return method as 'cash' | 'card' | 'stripe' | 'paypal' | 'virement';
    }
  };
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      // Récupérer le premier véhicule du panier pour créer la réservation
      const firstVehicle = cartItems[0];
      if (!firstVehicle) {
      ...
      const isoDateTime = dateStr && time
        ? new Date(`${dateStr}T${time}:00Z`).toISOString()
        : new Date().toISOString();

      // 📅 Créer la réservation avec les champs corrects du backend
      const bookingResponse = await bookingService.create({
        ...
        luggageCount: parseInt(params.get("luggageCount") || "0"),
        luggageDetails: extras ? [{ type: extras, quantity: 1 }] : [],
        babySeat: cartItems.some(item => item.vehicleId === "pmr"),
        pmr: false,
        notes: `Réservation ${firstVehicle.vehicleName}`,
        totalPrice: totalPrice,
      });

      console.log("✅ Réservation créée:", bookingResponse);

      // 💳 Créer le paiement avec les champs corrects du backend
      ...
      const errorMessage = err.data?.message || err.message || "Erreur lors du paiement";
          ...
          <div className="lg:col-span-2">
            <div className="bg-card rounded-2xl border border-border p-5 shadow-sm sticky top-24 space-y-5">
              ...
              <div className="flex items-center justify-center py-2">
                <img src={paymentLogos} alt="PayPal, Mastercard, Visa" className="h-12 object-contain" />
              </div>
              ...
              <Button
                onClick={handlePay}
                disabled={!paymentMethod || isLoading || !isAuthenticated}
              ...
              >
                {isLoading ? "Traitement..." : `Payer ${totalPrice} DT`}
