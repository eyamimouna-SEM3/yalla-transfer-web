...
import { ArrowLeft, Users, Luggage, Baby, Accessibility, ArrowRight, Building2, User, Star, Plus, Minus, ShoppingCart } from "lucide-react";
import { vehicleTypes, VehicleType } from "@/data/vehicleTypes";
import { motion, AnimatePresence } from "framer-motion";

const mockProviders = [
  { name: "TransTunisie Express", type: "company", rating: 4.8 },
  { name: "Sahara Tours", type: "company", rating: 4.5 },
  { name: "Mohamed Ben Ali", type: "driver", rating: 4.9 },
  { name: "VoyagePlus SARL", type: "company", rating: 4.3 },
  { name: "Ahmed Chauffeur VTC", type: "driver", rating: 4.7 },
  { name: "Carthage Transfer", type: "company", rating: 4.6 },
  { name: "Hamdi Slim", type: "driver", rating: 4.4 },
  { name: "MediterranéenTransport", type: "company", rating: 4.2 },
];

const getProvider = (index: number) => mockProviders[index % mockProviders.length];
...
export interface SelectedOffer {
  id: string;
  vehicle: VehicleType;
  provider: { name: string; type: string; rating: number };
  price: number;
  quantity: number;
}
...
const VehicleSelectionPage = () => {
  ...
  const offers = useMemo(() => matchingVehicles.flatMap((v, vi) => {
    const count = v.basePrice < 100 ? 3 : v.basePrice < 200 ? 2 : 1;
    return Array.from({ length: count }, (_, i) => ({
      vehicle: v,
      provider: getProvider(vi * 3 + i),
      price: v.basePrice + Math.round((i * 0.08 + Math.random() * 0.05) * v.basePrice),
      id: `${v.id}-${i}`,
    }));
  }).sort((a, b) => sortBy === "price" ? a.price - b.price : b.provider.rating - a.provider.rating), [matchingVehicles, sortBy]);
                  ...
                  >
                    <div className={`group w-full bg-card rounded-2xl border-2 text-left transition-all duration-300 overflow-hidden ${qty > 0 ? "border-primary shadow-lg" : "border-border hover:border-primary/30 hover:shadow-md"}`}>
                      ...
                      <div className="p-4 space-y-3">
                        <p className="text-sm font-semibold text-foreground">{offer.vehicle.description}</p>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground bg-muted/60 rounded-full px-2.5 py-1">
                            <Users className="h-3 w-3" /> {offer.vehicle.maxPassengers}
                          </span>
                          <span className="inline-flex items-center gap-1 text-[11px] text-muted-foreground bg-muted/60 rounded-full px-2.5 py-1">
                        ...
                        <div className="flex items-center justify-between pt-2 border-t border-border/50">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="h-7 w-7 rounded-full bg-muted flex items-center justify-center shrink-0">
                              {offer.provider.type === "company" ? <Building2 className="h-3.5 w-3.5 text-muted-foreground" /> : <User className="h-3.5 w-3.5 text-muted-foreground" />}
                            </div>
                            <div className="min-w-0">
                              <p className="text-xs text-foreground font-medium truncate">{offer.provider.name}</p>
                              <div className="flex items-center gap-1">
                                <Star className="h-3 w-3 text-accent fill-accent" />
                                <span className="text-[11px] text-muted-foreground">{offer.provider.rating}</span>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-2 pt-1">
                          <div className="flex items-center gap-1 bg-muted/60 rounded-lg border border-border">
                            <button
                              onClick={() => updateQuantity(offer.id, -1)}
                              disabled={qty === 0}
                              className="w-8 h-8 flex items-center justify-center rounded-l-lg hover:bg-muted transition-colors disabled:opacity-30"
                            >
                            ...
                            <button
                              onClick={() => updateQuantity(offer.id, 1)}
                              className="w-8 h-8 flex items-center justify-center rounded-r-lg hover:bg-muted transition-colors"
                            >
                          ...
                          <Button
                            onClick={() => {
                              if (qty === 0) updateQuantity(offer.id, 1);
                              else updateQuantity(offer.id, -qty);
                            }}
                            size="sm"
                            className="flex-1 h-8 text-xs font-semibold rounded-lg gap-1"
                            variant={qty > 0 ? "default" : "outline"}
                          >
                            {qty > 0 ? `${qty} sélectionné(s) ✕` : "Ajouter"}
                          </Button>
                        </div>
                      </div>
                    </div>
        ...
        {totalSelectedCount > 0 && (
          ...
          >
            <div className="container mx-auto px-4 lg:px-8 py-4 flex items-center justify-between gap-4">
              ...
              <Button onClick={handleReserve} className="rounded-xl px-6 font-semibold gap-2 bg-gradient-to-r from-primary to-accent text-primary-foreground">
