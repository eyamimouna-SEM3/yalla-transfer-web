...
const BookingForm = ({ compact }: { compact?: boolean }) => {
  ...
  // Auto-close logic: when both date and time are set after opening date popover
  const handleDateSelect = (d: Date | undefined) => {
    setDate(d);
    if (d) {
      setDateOpen(false);
      // Auto-open time picker right after date selection for fluidity
  ...
  const handleTimeChange = (h: string, m: string) => {
    setHours(h);
    setMinutes(m);
    // Close time popover automatically when both selected
    setTimeout(() => setTimeOpen(false), 200);
  };

  // Disable rule: réservation possible uniquement à partir de J+7 (7 jours après aujourd'hui)
  const isDateDisabled = (d: Date) => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const minBookable = new Date(today);
    minBookable.setDate(minBookable.getDate() + 7);
  ...
  const handleBook = () => {
    ...
    if (tripType === "round-trip" && !returnDate) {
      // Show error - return date is required for round trip
  ...
  return (
    <div className="bg-card rounded-2xl p-6 sm:p-8 shadow-card-hover border border-border/60 hover:shadow-2xl transition-shadow duration-500">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Popover open={dateOpen} onOpenChange={setDateOpen}>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              ...
              />
              <p className="px-4 pb-3 pt-1 text-[11px] text-muted-foreground border-t border-border">
                Réservation possible à partir de 7 jours à l'avance.
              </p>
            </PopoverContent>
          </Popover>
              ...
              <div className="flex gap-4 items-start">
                <div>
                  ...
                  <div className="grid grid-cols-6 gap-1 max-h-[180px] overflow-y-auto">
                    {hoursOptions.map((h) => (
                      <button key={h} onClick={() => handleTimeChange(h, minutes)} className={cn("w-8 h-8 rounded-lg text-xs font-medium transition-all", hours === h ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-muted text-foreground")}>{h}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-1">
                    {minutesOptions.map((m) => (
                      <button key={m} onClick={() => handleTimeChange(hours, m)} className={cn("w-8 h-8 rounded-lg text-xs font-medium transition-all", minutes === m ? "bg-primary text-primary-foreground shadow-sm" : "hover:bg-muted text-foreground")}>{m}</button>
                    ))}
                  </div>
                </div>
              </div>
          ...
          <Popover open={returnDateOpen} onOpenChange={setReturnDateOpen}>
            ...
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar
                ...
                disabled={(d) => isDateDisabled(d) || (date ? d < date : false)}
                initialFocus
                className="p-3 pointer-events-auto"
                modifiersClassNames={{ disabled: "text-muted-foreground/40 line-through" }}
              />
            </PopoverContent>
          </Popover>
        )}
        ...
        <Button onClick={handleBook} className="w-full py-3.5 text-base font-semibold rounded-xl shadow-button gap-2 bg-primary hover:bg-primary/90 text-primary-foreground transition-all duration-200 hover:shadow-lg">
