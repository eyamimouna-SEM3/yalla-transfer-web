...
interface Props {
  email: string;
  onBack: () => void;
  onCompleteWithPassword?: (password: string) => void;
  onComplete?: () => void;
}

const SPECIAL_REGEX = /[!@#$%^&*(),.?":{}|<>_\-\\/\[\];+=~`]/;

const StepPassword = ({ email, onBack, onCompleteWithPassword, onComplete }: Props) => {
  ...
  const handleComplete = () => {
    if (onCompleteWithPassword) {
      onCompleteWithPassword(password);
    } else if (onComplete) {
      onComplete();
    }
  };
  ...
  const getStrengthLabel = () => {
    ...
    if (!hasLetter) return { text: "Ajoutez au moins une lettre", color: "text-orange-500", icon: ShieldAlert };
    if (!hasNumber) return { text: "Ajoutez au moins un chiffre", color: "text-orange-500", icon: ShieldAlert };
    if (!hasSpecial) return { text: "Ajoutez un caractère spécial (ex : !@#$%)", color: "text-orange-500", icon: ShieldAlert };
      ...
      <div className="bg-card rounded-3xl border border-border shadow-[0_8px_40px_-12px_hsl(var(--primary)/0.1)] overflow-hidden">
        ...
        <div className="p-8 sm:p-10">
          ...
          <div className="space-y-5">
            ...
            {password.length > 0 && (
              <div className="space-y-2">
                <div className="flex gap-1.5">
                  ...
                  <div className={`h-1 flex-1 rounded-full transition-colors ${hasLetter ? "bg-green-500" : "bg-border"}`} />
                  <div className={`h-1 flex-1 rounded-full transition-colors ${hasNumber ? "bg-green-500" : "bg-border"}`} />
                  <div className={`h-1 flex-1 rounded-full transition-colors ${hasSpecial ? "bg-green-500" : "bg-border"}`} />
                </div>
                {strength && (
            ...
            <Button
              onClick={() => isValid && handleComplete()}
              disabled={!isValid}
