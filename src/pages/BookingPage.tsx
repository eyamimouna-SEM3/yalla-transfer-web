...
import { ArrowLeft, MapPin, Calendar, Clock, Users, Luggage, Accessibility, Baby, ArrowRight, ArrowLeftRight, Wifi, Droplets, ShieldCheck, Plus, Minus, Check } from "lucide-react";
...
const extras: ExtraOption[] = [
  { id: "water", label: "Eau fraîche", desc: "Bouteille 50cl pour le trajet", price: 1.5, unit: "DT / bouteille", icon: Droplets, quantitative: true },
...
const BookingPage = () => {
  ...
  const handleConfirm = () => {
    ...
    const extrasLabels = Object.entries(selectedExtras)
      .map(([id, qty]) => {
        const opt = extras.find(e => e.id === id);
        if (!opt) return "";
        return opt.quantitative ? `${opt.label} ×${qty}` : opt.label;
      })
      .filter(Boolean);
    allParams.set("extras", extrasLabels.join(","));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-card rounded-2xl border border-border p-6 shadow-card">
              <h3 className="font-display font-semibold text-foreground mb-1">Options supplémentaires</h3>
              <p className="text-xs text-muted-foreground mb-4">Cliquez sur une option pour l'activer ou la désactiver.</p>
              <div className="space-y-3">
                {extras.map((opt) => {
                  const qty = selectedExtras[opt.id] ?? 0;
                  const selected = qty > 0;
                  const Icon = opt.icon;
                  const lineTotal = opt.price * Math.max(qty, 1);
                  return (
                    <div
                      key={opt.id}
                      onClick={() => !opt.quantitative && toggleExtra(opt.id)}
                      role={opt.quantitative ? undefined : "button"}
                      className={`w-full flex items-center gap-3 p-4 rounded-xl text-left transition-all border ${
                        selected ? "bg-primary/5 border-primary/40 shadow-sm" : "bg-muted/40 border-transparent hover:bg-muted hover:border-border"
                      } ${!opt.quantitative ? "cursor-pointer" : ""}`}
                    >
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-colors ${selected ? "bg-primary text-primary-foreground" : "bg-background text-muted-foreground"}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-3">
                          ...
                          <span className="text-sm font-bold text-primary whitespace-nowrap">
                            {opt.quantitative ? `${opt.price} ${opt.unit ?? "DT"}` : `+${opt.price} DT`}
                          </span>
                        </div>
                        <p className="text-xs text-muted-foreground mt-0.5">{opt.desc}</p>
                        {opt.quantitative && qty > 0 && (
                          <p className="text-xs text-primary font-semibold mt-1">Sous-total : {lineTotal.toFixed(2)} DT</p>
                        )}
                      </div>

                      {opt.quantitative ? (
                        <div className="flex items-center gap-1.5 bg-background rounded-xl border border-border px-1 py-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                          <button
                            type="button"
                            onClick={() => updateExtraQty(opt.id, -1)}
                            disabled={qty === 0}
                            className="w-7 h-7 rounded-lg flex items-center justify-center text-foreground hover:bg-muted disabled:opacity-30 transition-colors"
                            aria-label="Diminuer"
                          >
                        ...
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 transition-all ${selected ? "bg-primary text-primary-foreground scale-100" : "bg-background border-2 border-border scale-90"}`}>
          ...
          <div className="lg:col-span-1">
            <div className="bg-card rounded-2xl border border-border p-6 shadow-card-hover sticky top-24 space-y-5">
              ...
              <Button onClick={handleConfirm} className="w-full py-3.5 text-base font-semibold rounded-xl shadow-button gap-2 bg-primary hover:bg-primary/90 text-primary-foreground">
