...
1. Crée un service `Web Service` dans Render.
2. Sélectionne ton dépôt Git.
3. Pour la racine du service, choisis `backend`.
4. Configure :
   - Build command : `npm install && npm run build`
   - Start command : `npm run start:prod`
5. Ajoute ces variables d'environnement dans Render :
   - `DATABASE_URL`
   - `JWT_SECRET`
   - `CORS_ORIGIN` (par exemple `*` ou l'URL de ton frontend)
   - `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` si tu utilises Google OAuth
   - `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_FROM` si tu utilises l'envoi d'e-mails

> Important : sur Render, ne laisse pas `localhost` dans `DATABASE_URL`. Utilise l'URL de la base de données distante fournie par Render ou par ton fournisseur PostgreSQL.
